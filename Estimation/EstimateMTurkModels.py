...
def loadData(type='car', subset='pooled'):
    file = r"Data\ChoiceData\MTurk\MTurkDataWeighted.xlsx"
    ...
    for specName, specVars in spec.items():
        for modelType in ['simple', 'mixed']:
            for weightName, weightCol in weights.items():
                ...
                modelID = 'mturk-{}-{}-{}-{}'.format(vehType, specName, modelType, weightName)
                if(modelType=='mixed'):
                ...
                with open('Models/mturkModels.dat', 'wb') as file:
                    pickle.dump(r, file)
                    file.close()

                with open('Models/mturkModels.dat', 'rb') as file:
