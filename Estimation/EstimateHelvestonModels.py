...
def loadData(type='car', subset='pooled'):
    file = r"Data\ChoiceData\Helveston\HelvestonDataWeighted.xlsx"
...
carSUVVarsPartworth = ['Acceleration', 'OpCost', 'bev75', 'bev100', 'bev150', 'phev10', 'phev20', 'phev40', 'hev', 'Chinese', 'American', 'SKorean', 'Japanese', 'BEV_NoFastCharge', 'PHEV_FastCharge']
carSUVVarsLinear = ['Acceleration', 'OpCost', 'bev', 'bevRangeRel', 'phev10', 'phev20', 'phev40', 'hev', 'Chinese', 'American', 'SKorean', 'Japanese', 'BEV_NoFastCharge', 'PHEV_FastCharge']
carSUVVarsAsinh = ['Acceleration', 'OpCost', 'bev', 'asinhBEVRange', 'phev10', 'phev20', 'phev40', 'hev', 'Chinese', 'American', 'SKorean', 'Japanese', 'BEV_NoFastCharge', 'PHEV_FastCharge']
carSUVSpecs = {'linear':carSUVVarsLinear, 'asinh':carSUVVarsAsinh}

specs = {'car':carSUVSpecs, 'suv':carSUVSpecs}

weights = {'weight2010':'Weight2010'}
...
# t = estimateModel(data['car'], carSUVVarsLinear, weightCol=weightCol, randSpec=randSpec, nDraws=numDraws)
# tBest = getBestModel(t)
# rFinal = prepareModelsToEnterDict(t)
# rFinal['rawModel'].summary()
# t = estimateModel(None, carSUVVarsLinear, weightCol=weightCol, startVals=startVals)
r = OrderedDict()
    ...
    for specName, specVars in spec.items():
        for modelType in ['simple', 'mixed']:
            for weightName, weightCol in weights.items():
                ...
                modelID = 'helveston-{}-{}-{}-{}'.format(vehType, specName, modelType, weightName)
                if(modelType=='mixed'):
                ...
                with open('Models/helvestonModels.dat', 'wb') as file:
                    pickle.dump(r, file)
                    file.close()

                with open('Models/helvestonModels.dat', 'rb') as file:
