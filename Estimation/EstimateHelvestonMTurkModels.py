...
def loadData(type='car', subset='pooled'):
    file = r"Data\ChoiceData\Helveston\HelvestonDataMTurkWeighted.xlsx"
...
carSUVSpecs = {'partworth':carSUVVarsPartworth, 'linear':carSUVVarsLinear, 'asinh':carSUVVarsAsinh}
    ...
    for specName, specVars in spec.items():
        for modelType in ['simple', 'mixed']:
            for weightName, weightCol in weights.items():
                ...
                modelID = 'helvestonMTurk-{}-{}-{}-{}'.format(vehType, specName, modelType, weightName)
                if(modelType=='mixed'):
                ...
                with open('Models/helvestonMTurkModels.dat', 'wb') as file:
                    pickle.dump(r, file)
                    file.close()

                with open('Models/helvestonMTurkModels.dat', 'rb') as file:
