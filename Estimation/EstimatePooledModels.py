...
def loadData(type='car', subset='pooled'):
    file = r"Data\ChoiceData\Pooled\PooledWeighted.xlsx"
    ...
    for specName, specVars in spec.items():
        for modelType in ['simple', 'mixed']:
            for weightName, weightCol in weights.items():
                ...
                modelID = 'pooled-{}-{}-{}-{}'.format(vehType, specName, modelType, weightName)
                if(modelType=='mixed'):
                ...
                with open('Models/pooledModels.dat', 'wb') as file:
                    pickle.dump(r, file)
                    file.close()

                with open('Models/pooledModels.dat', 'rb') as file:
